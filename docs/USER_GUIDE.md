...
- 💬 **多渠道接入**：不仅能用电脑命令行，还能用 Telegram、微信随时随地对话
...
> 💡 长期记忆还和**上下文压缩**联动：长会话自动压缩历史时，被丢弃的中间内容会抢救到记忆库，后续仍能通过召回找回——压缩不会真正"失忆"。也可用 `/compress [主题]` 手动压缩。
...
2. **登录成功后**，凭证会自动保存到 `.memory/weixin_account.json`
...
**注意事项**：
- 微信登录凭证会过期，过期后需要重新运行 `hachimi weixin`
- 服务运行期间重新登录后，微信渠道会自动加载新的凭证
- 同一时间只能有一个客户端使用这个微信账号
- 群聊中需要 @机器人 才会触发回复
- 微信渠道支持直接发送生成的图片/文件/视频附件；个别文件发送失败时，回复中会提供本地路径和 HTTP 下载路径兜底
...
| `/new` | 开始新的对话（清空当前对话） |
| `/compress [主题]` | 手动压缩上下文历史（可选主题，优先保留相关信息） |
| `/exit` | 退出程序 |
...
长会话默认在接近模型上下文上限时自动压缩历史。可调整触发时机或更换压缩摘要用的模型：
