...
def _build_executor_dynamic_context(deps: AgentDeps | None) -> str:
    """构造 executor 本轮动态 system prompt 段(executor 专用按需块 +
    时间/TaskFrame/记忆)。

    主动调用 ``build_executor_extra_dynamic_block(deps)`` 和
    ``build_system_dynamic_block(deps)`` 生成,与 executor agent 的
    ``@agent.system_prompt`` 钩子拼装顺序保持一致(executor 专用块在前、
    记忆/时间垫底),WebUI"展开运行时上下文"按钮才能看到与模型完全相同的快照。
